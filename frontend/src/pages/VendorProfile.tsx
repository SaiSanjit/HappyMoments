...
const VendorProfile = () => {
  ...
  useEffect(() => {
    const loadVendorData = async () => {
      ...
      try {
        ...
        try {
          const highlighted = await getHighlightedCatalogImages(parseInt(vendorData.vendor_id));
        ...
        try {
          const allImages = await getAllCatalogImages(parseInt(vendorData.vendor_id));
  ...
  const photographer = vendor ? {
    ...
    vendor: vendor // Correct placement at top level
  } : { ...defaultPhotographer, vendor: null };
  ...
  const openWhatsApp = () => {
    ...
    const whatsappNumber = photographer.contact.whatsapp.replace(/[^0-9]/g, '');
    if (!whatsappNumber) {
      alert('Sorry, this vendor does not have a valid WhatsApp number listed.');
      return;
    }

    const brandName = vendor?.brand_name || 'there';
    const message = `Hi ${brandName}, I just saw your amazing work on Happy Moments and I'm super interested in your services 📸
                      ...
                      <div className="flex justify-center mt-6 gap-3">
                        {photographer.highlights.map((_, index) => (
                          <button
                            ...
                            className={`w-4 h-4 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-blue-600 scale-125 shadow-lg' : 'bg-gray-300 hover:bg-gray-400'
                              }`}
                          />
                        ))}
                      </div>
                  ...
                  <CardContent className="p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {photographer.packages.map((pkg, index) => (
                        <div
                          key={index}
                          className={`relative p-8 rounded-2xl border-2 transition-all duration-300 hover:shadow-xl hover:-translate-y-2 ${pkg.popular
                        ...
                        >
                          ...
                          <Button
                            className={`w-full py-4 text-lg font-bold rounded-xl transition-all duration-300 ${pkg.popular
